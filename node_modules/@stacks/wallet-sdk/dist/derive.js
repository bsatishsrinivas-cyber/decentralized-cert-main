...
const selectDerivationTypeForUsername = async ({ username, rootNode, index, network, }) => {
    if (network) {
        const nameInfo = await (0, auth_1.getNameInfo)({ name: username });
        ...
        if (derivedAddress !== nameInfo.address) {
            const dataPrivateKey = (0, exports.deriveDataPrivateKey)({
                rootNode,
                index,
            });
            derivedAddress = (0, transactions_1.getAddressFromPrivateKey)(dataPrivateKey);
            if (derivedAddress !== nameInfo.address) {
...
const selectUsernameForAccount = async (opts) => {
    ...
    if (opts.network) {
        const stxPrivateKey = (0, exports.deriveStxPrivateKey)(opts);
        const address = (0, transactions_1.getAddressFromPrivateKey)(stxPrivateKey, opts.network);
        let username = await (0, usernames_1.fetchFirstName)({ address, client });
        if (username) {
            return { username, derivationType: DerivationType.Wallet };
        }
        else {
            const dataPrivateKey = (0, exports.deriveDataPrivateKey)(opts);
            const address = (0, transactions_1.getAddressFromPrivateKey)(dataPrivateKey, opts.network);
            username = await (0, usernames_1.fetchFirstName)({ address, client });
            if (username) {
...
const fetchUsernameForAccountByDerivationType = async (opts) => {
    const network = (0, network_1.networkFrom)(opts.network ?? 'mainnet');
    const client = Object.assign({}, (0, network_1.clientFromNetwork)(network), opts.client);
    const privateKey = (0, exports.derivePrivateKeyByType)(opts);
    const address = (0, transactions_1.getAddressFromPrivateKey)(privateKey, network);
    const username = await (0, usernames_1.fetchFirstName)({ address, client });
    return { username };
};
...
const deriveStxPrivateKey = ({ rootNode, index }) => {
    ...
    return (0, transactions_1.compressPrivateKey)(childKey.privateKey);
};
